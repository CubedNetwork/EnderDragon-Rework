...
# Summon arrow
execute as @e[tag=dragon_rework.miniboss] at @s if entity @s[tag=!dragon_rework.minibossMAD] anchored eyes facing entity @a[tag=dragon_rework.miniboss.arrow_beam.VICTIM.temp] eyes run summon arrow ^ ^ ^ {pickup:0b,damage:3.0d,crit:0b,Tags:["dragon_rework.arrow","dragon_rework.miniboss.arrow_beam","dragon_rework.miniboss.arrow_beam.temp","dragon_rework.remove"],SoundEvent:"entity.arrow.hit",item:{id:"minecraft:arrow",count:1,components:{"minecraft:potion_contents":{custom_effects:[{id:"minecraft:slowness",duration:600}]}}},weapon:{id:"minecraft:bow",count:1,components:{"minecraft:enchantments":{levels:{"minecraft:power":4,"minecraft:punch":3,flame:1,infinity:1}},"minecraft:unbreakable":{show_in_tooltip:false}}}}
